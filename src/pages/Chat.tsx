...
const Chat: React.FC = () => {
  ...
  const { data: mensagens, isLoading: loadingMensagens } = useMensagens(selectedConversa?.id || null);
  ...
  const handleNovaConversa = async (userId: string) => {
    ...
    // Encontrar ou criar objeto de conversa para selecionar
  ...
  // Filtrar usuários para nova conversa (admin/guardião pode falar com todos)
  const usuariosDisponiveis = allProfiles?.filter(p => {
    if (p.id === user?.id) return false;
    if (searchUser && !p.full_name?.toLowerCase().includes(searchUser.toLowerCase())) return false;
    return true;
  }) || [];
            ...
            {conversas.map(conversa => (
              ...
              >
                <Avatar className="w-12 h-12">
                  <AvatarImage src={conversa.outro_participante?.avatar_url || undefined} />
                  <AvatarFallback className="bg-primary/10 text-primary">
                    {conversa.outro_participante?.full_name?.charAt(0) || <User className="w-5 h-5" />}
                  </AvatarFallback>
                </Avatar>
  ...
  const AreaMensagens = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b flex items-center gap-3">
        ...
        <Avatar>
          <AvatarImage src={selectedConversa?.outro_participante?.avatar_url || undefined} />
          <AvatarFallback className="bg-primary/10 text-primary">
            {selectedConversa?.outro_participante?.full_name?.charAt(0) || <User className="w-4 h-4" />}
          </AvatarFallback>
        </Avatar>
        <div>
    ...
    <PageContainer maxWidth="xl" className="h-[calc(100vh-8rem)]">
      <Card className="h-full overflow-hidden">
        <div className="flex h-full">
          {/* Lista de conversas - esconde no mobile quando tem conversa selecionada */}
          <div className={cn(
            'border-r',
            isMobile 
              ? selectedConversa ? 'hidden' : 'w-full' 
              : 'w-80 shrink-0'
          )}>
            <ListaConversas />
          </div>
          ...
          <div className={cn(
            'flex-1',
            isMobile && !selectedConversa && 'hidden'
          )}>
            {selectedConversa ? (
              <AreaMensagens />
            ) : (
